...
import {ISubjectStatistic} from "../types/Statistic.js";

export const sendEmail = (userId:number, email:string, visitType:number, desc:string, s_name:string, date:string, name:string, statistic : ISubjectStatistic) => {
...
// Определение объекта письма
    const mailOptions = {
        from: 'no-reply-rsue@mail.ru', // адрес электронной почты отправителя
        to: `${email.trim()}`, // адрес электронной почты получателя
        subject: 'Уведомление о посещаемости', // тема письма
...
function mailHTML(visit:number, desc:string, s_name:string, date:string, name:string, statistic : ISubjectStatistic){
    return `
<div
      style="
